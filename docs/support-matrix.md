...
| Hygon Z100L | ROCm / DTK | device memory, all sizes to 512 MiB | yes | measured | 2026-09-18 |
...
| Feature | Verified on |
| --- | --- |
| In-place transfer, zero payload copies | RDMA (host and A40 memory), UCX, both directions |
...
- **GPU memory on Hygon, Cambricon and Moore Threads end to end.** Their
  registration limits and stream semantics are measured; transfers on those
  hosts have not been run through this library.
